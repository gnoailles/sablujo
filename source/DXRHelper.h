...
#define ROUND_UP(v, powerOf2Alignment) (((v) + (powerOf2Alignment)-1) & ~((powerOf2Alignment)-1))
    ...
    IDxcBlob* CompileShaderLibrary(LPCWSTR fileName)
    {
        ...
        {
            HMODULE CompilerDLL = LoadLibraryA("vendor/dxcompiler/dxcompiler.dll");
            Assert(CompilerDLL);
            DxcCreateInstance = (DxcCreateInstanceProc)GetProcAddress(CompilerDLL, "DxcCreateInstance");
            Assert(DxcCreateInstance);
        }
